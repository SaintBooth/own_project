# PromptSpace — Architecture & Product Specification

<!--
  Статус:   ✅ Approved — Ready for Development
  Версия:   2.3
  Дата:     2026-02-27
  Владелец: Tech Lead / System Architect
-->

> [!IMPORTANT]
> **Статус: ✅ Approved — Ready for Development**
> Версия **2.3** · Обновлено **2026-02-27** · Валюта: **только RUB**
...
| RPO | ≤ 5 мин |
| Стек | Django 5 · Next.js 15 · PostgreSQL 16 · Redis 7 · Vault HA |
| Комплаенс | 152-ФЗ, серверы в РФ |
...
**Изменения v2.0 (относительно v1.6):** Финальная редакция, Release. Структурные правки: `## История изменений` уровень скорректирован; нумерация всех подразделов (9.1–9.10, 10.1–10.13, 13.1–13.2, 20.1–20.3, 21.1–21.2); TOC дополнен навигацией по подпунктам; шапка документа переработана по best practice 2026 (GitHub callouts, badges, parameter table). Добавлены разделы 26 «Дорожная карта разработки» и 27 «Чеклист запуска в production».

**Изменения v2.1 (Frontend Completion):** Закрыты все пробелы frontend-спецификации (22 замечания из audit + 29 Q&A). Добавлены разделы 12.6–12.15: Design System (shadcn/ui + Tailwind v4, Plus Jakarta Sans/Geist/JetBrains Mono, Framer Motion), Error Handling UI (4× error.tsx + Sonner), Loading States (Skeleton + Spinner), KYC State Machine (middleware.ts protection), Prompt Content View (variables substitution + copy), File Upload UX (react-dropzone + XHR progress), Accessibility (WCAG 2.1 AA + axe-core CI), next.config.ts (security headers + CSP + standalone), State Management (3× Zustand stores + TanStack Query + nuqs), Страница автора /seller/{username} (avatar upload flow, bio, public profile card, OG Image). Добавлены: 10.13 API публичного профиля продавца, 10.9 avatar upload endpoints, SellerProfile.bio + avatar_url поля, 19.2 NEXT_PUBLIC_* переменные, 23.7 Browser polling уведомления, 17 OG ImageResponse + ISR таблица + Yandex.Metrika события, 18 Frontend CI (eslint + tsc + vitest + next build + bundle budget), router payment/result page, расширенное дерево frontend/ в разделе 13.

**Изменения v2.2 (относительно v2.1):** Точечные правки по аудиту спецификации: 12.13 — удалён `'unsafe-inline'`, реализован nonce-based CSP + Server-Side Tagging для Яндекс.Метрики; ADR-009 — добавлена зона `api_agent` (300r/m по токену) для M2M/Agentic AI Rate Limiting; 10.6 — Digital Provenance для изображений (C2PA / digital watermarking при confirm); 13.2 — Carbon-Aware Scheduling для некритичных периодических задач.

**Изменения v2.3 (относительно v2.2):** Tech Lead audit fixes (16 пунктов): 9.1 — добавлена модель `ApiKey` (M2M, SHA-256 хранение, индекс по `key_hash`); 9.4 — `Review` — добавлен `CheckConstraint(rating 1–5)` на уровне БД; 9.2 — `ModerationRecord.decision` — добавлены `APPEAL_APPROVED`/`APPEAL_REJECTED`; 9.1 — уточнена семантика `User.avatar_object_key` vs `SellerProfile.avatar_url`; 11.2 — KYC periodic check: `REJECTED` → `CANCELLED` (корректный статус потери НПД/ИП); ADR-009 — Nginx rate-limit переписан через `map`-блок (устранён `if is evil` антипаттерн; ключ зоны `api_agent` — хеш токена, не plaintext); 7/25.9 — зафиксирован атомарный порядок операций `published_at → content_hash` (новый раздел 25.9); ADR-016 — выбор AISP-провайдера (LLM Guard OSS self-hosted + fallback Yandex Foundation Models Moderation); 10 — `/docs` помечен как `dev/staging only`; 9.2 — документирована связь `variables` JSONField и `{{placeholder}}` в тексте промпта; 8.1 — Mermaid-диаграмма обновлена для Server-Side Tagging Метрики; 17 — ISR revalidate `/catalog`: 60 сек → 300 сек + `revalidatePath` при публикации; 26.6 — исправлена опечатка `Execução` → `Выполнение`.
...
10. [API Contract](#10-api-contract)
    ...
    - 10.4–10.12: Auth, Каталог, Промпты, Платежи, Медиа, Продавец, Модерация, Пользователь, Отзывы
    - 10.13 Публичный профиль продавца · 10.14 Webhook
11. [Интеграции](#11-интеграции)
12. [Frontend](#12-frontend)
    - 12.5 Performance · 12.6 Design System · 12.7 Error Handling
    - 12.8 Loading States · 12.9 KYC States · 12.10 Prompt Content View
    - 12.11 File Upload · 12.12 Accessibility · 12.13 next.config.ts
    - 12.14 State Management · 12.15 Страница автора
...
19. [Переменные окружения](#19-переменные-окружения)
    - 19.1 Backend · 19.2 Frontend (NEXT_PUBLIC_*)
...
23. [Система уведомлений](#23-система-уведомлений)
    - 23.1–23.6 Backend · 23.7 Browser (In-App) уведомления — Frontend
24. [Disaster Recovery Plan](#24-disaster-recovery-plan)
25. [Соглашения по реализации](#25-соглашения-по-реализации)
26. [Дорожная карта разработки](#26-дорожная-карта-разработки)
    - 26.1 Sprint 0 · 26.2 Sprint 1 · 26.3 Sprint 2 · 26.4 Sprint 3 · 26.5 Sprint 4 · 26.6 Sprint 5
27. [Чеклист запуска в Production](#27-чеклист-запуска-в-production)
...
> [!WARNING]
> **Telegram Bot API и 152-ФЗ:** серверы Telegram расположены вне РФ (Дублин / Амстердам). Telegram используется как **опциональный** push-канал (`TELEGRAM_ENABLED` env flag). В тело всех Telegram-сообщений **запрещено** включать ПДн: имена, email, суммы, названия промптов. Допустимо только: «Есть новое уведомление — войдите на сайт». DPA с Telegram не требуется при отсутствии ПДн в сообщениях. При недоступности (сбой, блокировка) — автоматический fallback на Email + in-app без потери функциональности (раздел 23).
...
> [!NOTE]
> Модели `PromptAccess` и `Review` используют `on_delete=models.PROTECT` для `User`, поскольку User не удаляется физически — происходит обезличивание. Каскадное удаление дочерних записей выполняется явно в задаче `anonymize_account`.
...
graph TB
    ...
    NextJS -->|"SST events (server-side)"| YaMetSST["Яндекс.Метрика<br/>(Server-Side Tagging)"]
```
...
class SellerProfile(models.Model):
    ...
    # Публичный профиль
    bio = models.TextField(max_length=500, blank=True)            # публичное BIO автора
    avatar_url = models.CharField(max_length=500, blank=True)     # URL в YOS/CDN
...
class ApiKey(models.Model):
    """M2M API-ключи для Agentic AI (раздел 5.7).
    Plaintext ключ показывается пользователю один раз при создании и нигде не сохраняется.
    В БД хранится только SHA-256 хеш для верификации входящих запросов.
    """
    id = models.UUIDField(primary_key=True, default=uuid4)
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="api_keys")
    name = models.CharField(max_length=100)                 # "Prod Agent", "Staging"
    # SHA-256(plaintext_key) — уникальный, индексированный для O(1) lookup
    key_hash = models.CharField(max_length=64, unique=True)
    # Первые 8 символов после префикса — для отображения в UI ("ps_live_abc•••")
    key_preview = models.CharField(max_length=20)
    created_at = models.DateTimeField(auto_now_add=True)
    last_used_at = models.DateTimeField(null=True, blank=True)  # обновляется при каждом использовании
...
> [!NOTE]
> **Аватар: `User.avatar_object_key` vs `SellerProfile.avatar_url`**
>
> Это два разных поля для двух разных сущностей:
> - `User.avatar_object_key` — **ключ объекта в YOS** (например, `avatars/{user_id}/{uuid}.jpg`). Используется для **универсального** аватара пользователя (любой роли). Поле хранит YOS object_key, а не публичный URL — URL генерируется при отдаче через CDN/presigned.
> - `SellerProfile.avatar_url` — **публичный CDN URL** аватара продавца, отображаемый на публичном профиле (`/seller/{username}/`). Обновляется эндпоинтами `POST /seller/avatar/confirm/` и `DELETE /seller/avatar/`. При загрузке: object_key → magic bytes check → CDN URL → сохранить в `SellerProfile.avatar_url`.
>
> **Правило:** `User.avatar_object_key` — для внутреннего использования; `SellerProfile.avatar_url` — для публичного отображения. В `/users/me/` возвращается URL, вычисленный из `User.avatar_object_key` (через CDN base URL). Если у пользователя есть `SellerProfile`, публичный профиль использует `SellerProfile.avatar_url`.
...
class Prompt(models.Model):
    ...
    #
    # Связь с текстом контента:
    # Продавец размечает переменные в тексте промпта плейсхолдерами {{TOPIC}}.
    # При создании/редактировании промпта фронтенд извлекает плейсхолдеры regex /\{\{(\w+)\}\}/g
    # и предлагает продавцу заполнить описание и пример для каждого — они сохраняются в этом поле.
    # При просмотре (GET /prompts/{id}/content/) возвращаются оба: зашифрованный текст
    # (с {{TOPIC}}) и массив variables для рендера интерактивной формы подстановки на фронте.
    # Контракт: имена переменных в тексте и в массиве variables ДОЛЖНЫ совпадать.
    # Валидация на бэкенде при submit: set(regex matches) == set(v["name"] for v in variables).
...
class ModerationRecord(models.Model):
    ...
    decision = models.CharField(max_length=30, choices=[
        ...
        # Апелляции: APPEAL_REQUESTED → модератор принимает одно из двух решений
        ("APPEAL_APPROVED", "Appeal Approved → PENDING_MODERATION"),
        ("APPEAL_REJECTED", "Appeal Rejected → BLOCKED_BY_SECURITY"),
    ])
...
class Review(models.Model):
    ...
    class Meta:
        unique_together = [("buyer", "prompt")]
        constraints = [
            # CheckConstraint — единственная надёжная гарантия на уровне БД.
            # clean() вызывается только Django Admin / Form.full_clean();
            # прямой .save() и .objects.create() минуют clean() полностью.
...
> [!WARNING]
> **`/api/v1/docs` — только dev / staging.** В production: `DJANGO_NINJA_DOCS_URL=` (пустое значение) — маршрут не регистрируется. Открытая Swagger-документация в production раскрывает схемы API атакующим. Проверка — в чеклисте раздела 27.2.
...
| POST | `/prompts/{id}/examples/confirm/` | seller | `{ "object_key": "..." }` — подтвердить загрузку. **Безопасность:** не доверять `HeadObject` (ContentType контролируется клиентом, возможна подмена MIME → XSS). Backend скачивает первые 2048 байт (`GetObject Range="bytes=0-2048"`), проверяет **magic bytes** через `python-magic`; whitelist `{image/jpeg, image/png, image/webp, image/gif}`. При несоответствии — удаляет файл + 422. `ContentLength` ≤ 5 МБ. **Digital Provenance (C2PA / watermarking):** при успешной валидации — встраивание невидимого цифрового водяного знака (digital watermarking, например, через `imwatermark` или Yandex Cloud Image Optimization API) с метаданными `{prompt_id, seller_id, timestamp}`. Это обеспечивает криптографическую привязку изображения к автору — защита от распространения примеров без указания источника. |
...
| GET | `/seller/profile/` | seller | Профиль + kyc_status, split_enabled, bio, avatar_url |
| PATCH | `/seller/profile/` | seller | `{ "bio"?: str(≤500), "robokassa_merchant_login"?: str }` — обновление профиля |
| POST | `/seller/avatar/presigned/` | seller | Запрос presigned PUT URL для загрузки аватара в YOS. Body: `{ "content_type": "image/jpeg\|image/png\|image/webp", "size": int(≤2097152) }` → `{ "upload_url": "...", "object_key": "avatars/{user_id}/{uuid}.ext" }` |
| POST | `/seller/avatar/confirm/` | seller | После успешного PUT в YOS: `{ "object_key": "..." }` → backend верифицирует magic bytes, обновляет `SellerProfile.avatar_url`, возвращает CDN URL |
| DELETE | `/seller/avatar/` | seller | Удаление аватара; `SellerProfile.avatar_url = ""` + удаление файла из YOS |
...
### 10.13. Эндпоинты: Публичный профиль продавца

| Метод | Путь | Auth | Описание |
|-------|------|------|----------|
| GET | `/seller/{username}/public/` | — | Публичный профиль продавца: `{ username, avatar_url, bio, stats: { prompts_count, total_sales, avg_rating, member_since } }` |
| GET | `/seller/{username}/prompts/` | — | PUBLISHED промпты продавца; cursor-пагинация `?cursor=&limit=12`; параметр `sort=newest\|price_asc\|price_desc` |
...
- **Жизненный цикл KYC:** Celery Beat `verify_active_kyc_status` (раз в 7 дней для VERIFIED продавцов). При потере статуса НПД/ИП (Dadata возвращает неактивный статус) → `kyc_status=CANCELLED` (не `REJECTED`! `CANCELLED` — именно для случая утраты ранее подтверждённого статуса; `REJECTED` — только при первичном отклонении); уведомление продавцу (`KYC_STATUS_LOST`); `on_kyc_cancelled` аннулирует PENDING транзакции → `CANCELLED`.
...
│   ├── library/page.tsx
│   ├── purchases/page.tsx
│   └── payment/result/page.tsx     # /payment/result — статус платежа, polling до SUCCESS
├── (seller)/
...
**UI компоненты:** `shadcn/ui` — копируемые компоненты (не зависимость), Tailwind CSS v4.

**Темизация:**
- Тема по умолчанию — **dark**. Переключение через `next-themes`.
- CSS variables в `globals.css` — единый источник цветов, радиусов, теней.
- Никаких инлайн-стилей в компонентах; только `className` с Tailwind.
...
export const headingFont = Plus_Jakarta_Sans({ subsets: ['latin', 'cyrillic'], weight: ['600', '700', '800'] })
export const bodyFont = Geist({ subsets: ['latin'] })
export const monoFont = JetBrains_Mono({ subsets: ['latin'], variable: '--font-mono' })
```

- **Plus Jakarta Sans** — заголовки (H1–H4), акцентные кнопки.
- **Geist** — основной текст, описания, UI.
- **JetBrains Mono** — содержимое промптов (`<pre>`), code snippets, переменные.

**Анимации (Framer Motion):**
- Принцип: минимализм. Одна хорошо срежиссированная анимация лучше 10 случайных.
- `initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}` — page-level reveal.
- `AnimatePresence` — для модальных окон, уведомлений.
- Hover-эффекты на карточках промптов: `whileHover={{ scale: 1.02 }}` (не более).
- Запрет на анимации при `prefers-reduced-motion` (через Framer Motion `MotionConfig`).

### 12.7. Error Handling UI

**Стратегия: 4 файла `error.tsx`** (по одному на route group):

| Файл | Покрывает |
|------|-----------|
| `app/(public)/error.tsx` | Каталог, категории, страницы промптов |
| `app/(buyer)/error.tsx` | Библиотека, покупки, платёж |
| `app/(seller)/error.tsx` | Дашборд, редактор промптов, KYC |
| `app/(moderator)/error.tsx` | Очередь модерации |

**HTTP Error → UI State маппинг:**

| HTTP код | Компонент / поведение |
|----------|----------------------|
| 401 | Redirect `/login` (middleware.ts) |
| 403 | `<ForbiddenState>` — "Доступ закрыт" |
| 404 | `not-found.tsx` с поиском / возвратом в каталог |
| 422 | Inline field error (react-hook-form setError) |
| 429 | `<RateLimitState>` — "Слишком много запросов, подождите N сек" (Retry-After header) |
| 500 / сеть | `<ErrorState>` — "Что-то пошло не так" + кнопка retry (`reset()` из error.tsx) |
...
// Успех
toast.success('Промпт куплен! Открываю библиотеку...')

// Ошибка
toast.error('Не удалось оформить покупку', { description: error.detail })

// Копирование
toast('Скопировано в буфер обмена', { duration: 2000, icon: '✓' })
```

### 12.8. Loading States

**Принцип:** skeleton для контента с размером (каталог, карточки) — предотвращает CLS. Spinner для действий без предсказуемого размера (платёж, KYC, загрузка файла).

**Каталог — shadcn Skeleton:**
```tsx
// components/PromptCardSkeleton.tsx
export function PromptCardSkeleton() {
  return (
    <div className="rounded-xl border p-4 space-y-3">
      <Skeleton className="h-40 w-full rounded-lg" />  {/* изображение */}
      <Skeleton className="h-4 w-3/4" />               {/* title */}
      <Skeleton className="h-3 w-1/2" />               {/* автор */}
      <div className="flex justify-between">
        <Skeleton className="h-5 w-16" />              {/* цена */}
        <Skeleton className="h-8 w-20" />              {/* кнопка */}
      </div>
    </div>
  )
}
...
**Страницы с `loading.tsx`:** Next.js автоматически показывает `loading.tsx` как Suspense fallback при навигации.

| Route | loading.tsx содержит |
|-------|---------------------|
| `/catalog` | `<CatalogGridSkeleton>` (12 карточек) |
| `/prompt/[slug]` | `<PromptDetailSkeleton>` |
| `/seller/[username]` | `<AuthorProfileSkeleton>` |
| `/seller/dashboard` | `<DashboardSkeleton>` |

**Платёж / KYC / Upload:** `<Spinner>` (SVG animate-spin) + текстовый статус ("Обрабатываем платёж...", "Проверяем документы...").

### 12.9. KYC Onboarding States

**State machine:**

```
idle ──[submit form]──► submitted
submitted ──[Celery task]──► PENDING ──[polling 10s]──► VERIFIED ✅
                                                       └──► REJECTED ❌
VERIFIED ──[Celery Beat 7d check]──► CANCELLED (статус НПД/ИП потерян) ⚠️
```
...
const { data } = useQuery({
  queryKey: ['kyc-status'],
  queryFn: () => fetch('/api/v1/seller/profile/').then(r => r.json()),
  refetchInterval: (data) =>
    data?.kyc_status === 'PENDING' ? 10_000 : false,  // polling только при PENDING
})
```

**UI States:**

| kyc_status | Компонент |
|------------|-----------|
| `idle` (нет SellerProfile) | Форма: ИНН + тип (ИП/НПД) + кнопка «Подать заявку» |
| `PENDING` | Spinner + "Проверяем ИНН через Dadata..." + polling |
| `VERIFIED` | ✅ Badge "KYC верифицирован" + дата верификации |
| `REJECTED` | ❌ Причина + кнопка «Подать повторно» |
| `CANCELLED` | ⚠️ "Статус НПД/ИП утерян" + инструкция по восстановлению |
...
export function middleware(request: NextRequest) {
  ...
  if (
    request.nextUrl.pathname.startsWith('/seller/prompts/new') &&
    kycStatus !== 'VERIFIED'
  ) {
...
> [!NOTE]
> `kyc_status` в cookie — публичное состояние UI, не секрет. Фактическая проверка прав — на бэкенде при каждом API-запросе.

### 12.10. Просмотр содержимого промпта

**Правила отображения:**
- Контент промпта — **plain text** в `<pre>` + `JetBrains Mono`. HTML и Markdown не рендерятся. XSS невозможен.
- **Переменные (опционально):** если продавец обозначил переменные в формате `{{variable_name}}`, UI отображает интерактивную форму подстановки.

**Компонент с подстановкой переменных:**
```tsx
// components/PromptContentView.tsx
export function PromptContentView({ content }: { content: string }) {
  const variables = extractVariables(content)  // regex /\{\{(\w+)\}\}/g
  const [values, setValues] = useState<Record<string, string>>({})

  const rendered = variables.length
    ? content.replace(/\{\{(\w+)\}\}/g, (_, k) => values[k] || `{{${k}}}`)
    : content
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative">
        <pre className="font-mono text-sm bg-muted p-4 rounded-lg overflow-auto whitespace-pre-wrap">
          {rendered}
        </pre>
        <CopyButton content={rendered} />
      </div>
    </div>
  )
}
...
function CopyButton({ content }: { content: string }) {
  ...
  return (
    <button onClick={copy} className="absolute top-2 right-2 ...">
...
### 12.11. File Upload UX

**Библиотека:** `react-dropzone` — drag & drop + click to select.

**Клиентская валидация (до presigned URL запроса):**
- MIME: `image/jpeg`, `image/png`, `image/webp`, `image/gif`
- Размер: ≤ 5 МБ на файл
- Количество: ≤ 5 файлов на промпт

**Upload flow с прогрессом:**
```typescript
async function uploadFile(file: File, uploadUrl: string, onProgress: (pct: number) => void) {
...
|-----------|-------------|
| idle | Dropzone с иконкой + "Перетащите файлы или нажмите" |
| validating | Inline error под dropzone (неверный формат / размер) |
| uploading | Progress bar 0–100% + имя файла |
| success | ✅ Превью изображения + кнопка удаления |
...
|-----------|------------|
| Contrast ratio ≥ 4.5:1 (text) | Проверка в дизайн-системе (shadcn dark theme) |
| Keyboard navigation | Все интерактивные элементы через Tab; shadcn компоненты headless |
| Focus indicators | `focus-visible:ring-2` — Tailwind (не скрывать outline) |
| ARIA labels | `aria-label` на icon buttons (NotificationBell, CopyButton, Upload) |
| Skip link | `<a href="#main-content" className="sr-only focus:not-sr-only">` в `layout.tsx` |
| Form errors | `aria-describedby` связан с error message; `aria-invalid` при ошибке |
| Images | `alt` обязателен; декоративные — `alt=""` |

**Автоматическое тестирование (CI):**
```yaml
- name: Accessibility audit
  run: npx @axe-core/playwright
  # Запускается после `next start` на staging build
  # Smoke test: /, /catalog, /prompt/[slug-example]
  # CI fail при любом violation уровня critical или serious
```

### 12.13. `next.config.ts`
...
const nextConfig: NextConfig = {
  output: 'standalone',                    // оптимальный Docker-образ
  ...
  images: {
    remotePatterns: [
      { protocol: 'https', hostname: 'storage.yandexcloud.net' },  // YOS прямой
      { protocol: 'https', hostname: '*.cdn.yandex.net' },         // Yandex CDN
    ],
    ...
    return [
      {
        ...
        headers: [
          ...
          { key: 'Referrer-Policy', value: 'strict-origin-when-cross-origin' },
          {
            ...
            value: [
              "default-src 'self'",
              "script-src 'self' https://mc.yandex.ru",  // Metrika — nonce-based (см. примечание)
              "img-src 'self' data: https://storage.yandexcloud.net https://*.cdn.yandex.net",
              "connect-src 'self' https://api.promptspace.ru https://mc.yandex.ru",
...
> [!NOTE]
> **CSP и Yandex.Metrika:** `'unsafe-inline'` удалён. Применяется **nonce-based CSP** через Next.js Middleware: `nonce` генерируется на каждый запрос (`crypto.randomUUID()`), прокидывается в headers и в кастомный `_document`/`<Script nonce={nonce}>`. Метрика инициализируется через Next.js Route Handler (`/api/v1/telemetry/metrika`) — **Server-Side Tagging**: клиент отправляет события на собственный API-роут, который проксирует данные в Яндекс.Метрику без загрузки стороннего скрипта. Это полностью устраняет CSP-компромисс.

### 12.14. State Management

**Архитектура: Zustand (client state) + TanStack Query (server state) + nuqs (URL state)**

```
┌─────────────────────────────────────────────────────────┐
│                    State Management                      │
│                                                          │
│  Zustand stores          TanStack Query     nuqs         │
│  ├─ authStore            ├─ prompts         ├─ filters   │
│  ├─ notificationsStore   ├─ purchases       └─ sort      │
│  └─ catalogFiltersStore  ├─ profile                      │
│     (sync → nuqs)        └─ notifications               │
└─────────────────────────────────────────────────────────┘
```

**authStore** — глобальное состояние аутентификации:
```typescript
// stores/authStore.ts
interface AuthStore {
...
**notificationsStore** — счётчик и список непрочитанных (см. 23.7).

**catalogFiltersStore + nuqs** — фильтры каталога синхронизированы с URL:
```typescript
// stores/catalogFiltersStore.ts (через nuqs)
import { parseAsString, parseAsInteger, useQueryStates } from 'nuqs'

export function useCatalogFilters() {
  return useQueryStates({
    category: parseAsString.withDefault(''),
    ai_model: parseAsString.withDefault(''),
    price_max: parseAsInteger,
    sort: parseAsString.withDefault('relevance'),  // relevance | price_asc | price_desc | newest
  })
}
```
...
const queryClient = new QueryClient({
  defaultOptions: {
    queries: {
      staleTime: 60_000,         // 1 мин по умолчанию
...
Специфичные `staleTime` по запросам: аналитика продавца — 5 мин; уведомления — polling (30 сек интервал); каталог — 1 мин.

### 12.15. Страница автора (`/seller/{username}`)

#### Концепция

Публичная страница автора — точка входа для шеринга. Авторы получают **уникальный URL** `https://promptspace.ru/seller/{username}`, которым могут делиться в соцсетях, Telegram, портфолио. Страница индексируется поисковиками (SSR + OG Image).

#### URL и SEO

- **URL:** `/seller/{username}` — уже в роутинге (`app/(public)/seller/[username]/page.tsx`)
- **`username`** — уникальный, задаётся при регистрации, не меняется (изменение — отдельный flow вне MVP)
- **OG Image:** `opengraph-image.tsx` — аватар + имя + tagline (первые 60 символов bio)
- **JSON-LD:** `Person` schema — `name`, `url`, `description` (bio)
- **robots:** индексируется (`/seller/` в allowlist `robots.txt`)
- **ISR:** `revalidate = 600` + `revalidatePath('/seller/{username}')` при обновлении профиля

#### Дизайн страницы

```
┌─────────────────────────────────────────┐
│ [AVATAR 96px]  username                 │
│                @username                │
│                ⭐ 4.8  |  📦 42 промпта │
│                ❤️ 128 продаж            │
│                👤 На платформе с мар. 2025│
│                                         │
│  Bio (до 500 символов, wrap)            │
│                                         │
│  [Поделиться URL] [Написать в Telegram] │
├─────────────────────────────────────────┤
│  Промпты автора (публичные, PUBLISHED)  │
│  ┌───────┐ ┌───────┐ ┌───────┐         │
│  │ card  │ │ card  │ │ card  │         │
│  └───────┘ └───────┘ └───────┘         │
│  [Загрузить ещё] (cursor pagination)    │
└─────────────────────────────────────────┘
```

#### Данные на странице (API)

```typescript
// GET /api/v1/seller/{username}/public/  (новый эндпоинт)
interface SellerPublicProfile {
  username: string
  avatar_url: string | null
  bio: string
  stats: {
    prompts_count: number      // PUBLISHED промпты
    total_sales: number        // кол-во успешных покупок
    avg_rating: number | null  // средний рейтинг (null если нет отзывов)
    member_since: string       // ISO date — created_at
  }
  telegram_username?: string   // если продавец указал (опционально)
}
```

> [!NOTE]
> Эндпоинт `GET /seller/{username}/public/` — публичный (без auth). Добавить в раздел 10 API.

```typescript
// GET /api/v1/seller/{username}/prompts/?cursor=&limit=12  (новый параметр на существующем)
// Возвращает только PUBLISHED промпты этого автора
```

#### Аватар — Upload Flow

```
1. Пользователь выбирает файл в настройках профиля (/seller/dashboard/settings)
2. Клиент: client-side валидация (MIME image/*, ≤ 2MB)
3. POST /api/v1/seller/avatar/presigned/ → { upload_url, object_key }
4. Клиент: PUT {upload_url} с файлом (XHR + progress bar)
5. POST /api/v1/seller/avatar/confirm/ с { object_key }
6. Backend: GetObject Range bytes=0-2048 → magic bytes проверка
7. Backend: обновляет SellerProfile.avatar_url → CDN URL
8. Клиент: обновляет превью аватара (invalidate TanStack Query)
```

**Аватар в UI:**
- Размер на странице автора: 96×96 px, `rounded-full`, `object-cover`
- Fallback при отсутствии: генерируемый аватар из первой буквы username (CSS `background + text`)
- `next/image` с `sizes="96px"` и `priority` (above fold)

#### Компонент страницы автора

```tsx
// app/(public)/seller/[username]/page.tsx
export default async function SellerPage({ params }: { params: { username: string } }) {
  const profile = await fetchSellerProfile(params.username)   // Server Component
  if (!profile) notFound()

  return (
    <main id="main-content">
      <SellerProfileCard profile={profile} />
      <section aria-label="Промпты автора">
        <Suspense fallback={<CatalogGridSkeleton count={12} />}>
          <SellerPromptsList username={params.username} />
        </Suspense>
      </section>
    </main>
  )
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `${profile.username} — промпты на PromptSpace`,
    description: profile.bio || `Промпты от ${profile.username}`,
    openGraph: {
...
function ShareButton({ username }: { username: string }) {
  const url = `https://promptspace.ru/seller/${username}`
  const copy = () => {
...
│   │   │   └── seller/[username]/
│   │   │       ├── page.tsx                     # публичная страница автора
│   │   │       └── opengraph-image.tsx
│   │   ├── (auth)/login/  (auth)/register/
│   │   ├── (buyer)/
│   │   │   ├── library/  purchases/
│   │   │   └── payment/result/page.tsx          # polling статуса платежа
│   │   ├── (seller)/
│   │   │   ├── dashboard/page.tsx               # аналитика
│   │   │   ├── prompts/new/  prompts/[id]/edit/
│   │   │   └── kyc/page.tsx
│   │   ├── (moderator)/moderation/page.tsx
│   │   ├── legal/  (privacy, terms, dsr)
│   │   ├── layout.tsx                           # root layout + Providers
│   │   ├── loading.tsx                          # root loading.tsx
│   │   ├── not-found.tsx
│   │   ├── error.tsx                            # root error.tsx
│   │   ├── fonts.ts                             # next/font конфигурация
...
│   │   └── utils.ts
│   ├── middleware.ts                             # KYC route protection
│   ├── next.config.ts
│   ├── tailwind.config.ts
...
> ¹ **Carbon-Aware Scheduling:** задачи с пометкой ¹ — некритичные ресурсоёмкие операции. Рекомендуется запускать их в часы минимальной углеродной интенсивности энергосети ЦОД (обычно ночные часы, 01:00–05:00 UTC по Yandex Cloud). В production — использовать Carbon-Aware Job Scheduler (или динамически сдвигать расписание через `django-celery-beat` API) при наличии данных о carbon intensity от провайдера. Мониторинг SCI-метрик (Software Carbon Intensity) выводится в OpenTelemetry дашборд.
...
# M2M / Agentic AI: отдельная зона по хешу токена, а не по IP.
# ВАЖНО: нельзя использовать $http_authorization напрямую — plaintext токен
# оказался бы в shared memory Nginx и потенциально в access-логах.
# map вычисляет переменную один раз за запрос; хеш MD5 необратим.
map $http_authorization $api_key_hash {
    default                   "";
    "~^Api-Key\s+(.+)$"      $1;  # извлекаем сам токен из заголовка
}
# Используем $api_key_hash как ключ зоны (хеш вычисляется Nginx hash_function,
# хранится только в виде fixed-size bucket — plaintext не сохраняется в памяти)
limit_req_zone $api_key_hash zone=api_agent:20m rate=300r/m;
...
location /api/v1/library/acquire/ {
    limit_req zone=api_acquire burst=3 nodelay;
    proxy_pass http://django_upstream;
}
# Для /prompts/ применяем agent-зону только если заголовок содержит Api-Key.
# Используем map-переменную (не if!), чтобы избежать антипаттерна "if is evil".
# Если $api_key_hash пустой — api_agent зона не срабатывает (limit_req_zone игнорирует пустой ключ).
location /api/v1/prompts/ {
    limit_req zone=api_agent  burst=100 nodelay;  # для Api-Key запросов
    limit_req zone=api_general burst=20 nodelay;  # для Bearer JWT запросов
    proxy_pass http://django_upstream;
}
location /api/ {
...
> [!NOTE]
> **Почему `map` вместо `if`:** директива `if` внутри `location` в Nginx — известный антипаттерн ("If is Evil"). При использовании `if` с `limit_req` обе зоны применяются одновременно вне зависимости от результата условия. `map` вычисляет переменную **до** обработки `location`, что даёт предсказуемое поведение. При пустом значении `$api_key_hash` (Bearer JWT запрос) Nginx `limit_req_zone` пропускает bucket с пустым ключом, фактически отключая зону `api_agent` для таких запросов.

**M2M Rate Limiting:** запросы с `Authorization: Api-Key ps_live_...` тарифицируются по зоне `api_agent` (300r/m на токен). Это предотвращает случайный «DDoS по неосторожности» со стороны Agentic Workflows (MAS), которые способны генерировать тысячи запросов в секунду. При превышении лимита — 429 с заголовком `Retry-After`. Высоконагруженные M2M-клиенты должны согласовывать увеличенный лимит через Superadmin.
...
**ADR-016: AISP-провайдер для SECURITY_SCAN**
**Status:** Accepted

**Context:** SECURITY_SCAN (раздел 7) требует автоматической проверки текста промпта на Prompt Injection, Jailbreaks, токсичный контент перед попаданием к модератору. Нужен конкретный провайдер — без этого задача Sprint 5 ("подключить AISP") не имеет чёткого технического задания.

**Ограничения:**
- 152-ФЗ: текст промпта содержит ИС продавца. Нельзя отправлять в зарубежные API (OpenAI Moderation, Azure Content Safety).
- Данные обрабатываются внутри РФ.
- Latency добавки к пайплайну публикации: допустимо до 10 сек (Celery-задача, асинхронно).

**Decision:** Двухуровневая архитектура:

| Уровень | Провайдер | Назначение |
|---------|-----------|------------|
| Primary | **LLM Guard OSS** (self-hosted, MIT license) | Детекция Prompt Injection, Jailbreaks, токсичного контента — работает локально, данные не покидают инфраструктуру. Модели: `PromptInjectionV2`, `ToxicityModel`. |
| Fallback | **Yandex Foundation Models Moderation API** (YandexGPT) | При недоступности LLM Guard (OOM, cold start). Данные остаются в РФ (Yandex Cloud). |

**Переменные окружения:**
```
SECURITY_SCAN_PROVIDER=llmguard           # llmguard | yandex
SECURITY_SCAN_API_URL=http://llmguard:8080 # для self-hosted llmguard контейнера
SECURITY_SCAN_API_KEY=                    # для yandex fallback: IAM-токен
SECURITY_SCAN_THRESHOLD=0.85             # порог вероятности для блокировки (0.0–1.0)
```

**Архитектура LLM Guard в docker-compose:**
```yaml
llmguard:
  image: ghcr.io/protectai/llm-guard:latest
  environment:
    - MODEL_PATH=/models
  volumes:
    - llmguard_models:/models   # модели скачиваются при первом запуске
  deploy:
    resources:
      limits:
        memory: 4G
...
**Интерфейс вызова (ISecurityScanProvider):**
```python
class ISecurityScanProvider(Protocol):
    def scan(self, text: str) -> tuple[bool, float]:
        """Возвращает (is_safe: bool, confidence: float)."""
        ...
```
Реализации: `LLMGuardProvider`, `YandexModerationProvider`. Переключение через `SECURITY_SCAN_PROVIDER` env.

**Consequences:**
- LLM Guard self-hosted — zero egress для текстов промптов; требует дополнительного контейнера (~2–4 GB RAM)
- Fallback на Yandex обеспечивает availability при проблемах с LLM Guard
- При недоступности обоих провайдеров → `SECURITY_SCAN` задача ретраится (max 3 попытки с exponential backoff); если все исчерпаны → алерт Sentry + промпт возвращается в `DRAFT`, продавцу уведомление

**Review date:** при появлении российского облачного решения класса Azure Content Safety — пересмотреть.
...
**OG Image — динамическая генерация (Next.js ImageResponse):**
- Файл: `app/(public)/prompt/[slug]/opengraph-image.tsx`
- Runtime: Edge (не Node.js) — холодный старт < 50ms.
- Контент: название промпта + категория + иконка PromptSpace + цена.
- Аналогично: `app/(public)/seller/[username]/opengraph-image.tsx` — аватар автора + bio (первые 80 символов) + количество промптов.
- CDN-кешируется через `next/headers` `Cache-Control: public, max-age=86400, stale-while-revalidate=3600`.

**ISR — стратегия ревалидации:**

| Страница | `revalidate` | Обоснование |
|----------|-------------|-------------|
| `/catalog` | 300 сек + `revalidatePath('/catalog')` при публикации/архивации | Баланс свежести и нагрузки: 60 сек = ~1000 revalidations/мин при 1000 RPS, что избыточно для MVP |
| `/category/[slug]` | 300 сек | Умеренная частота |
| `/prompt/[slug]` | 300 сек + `revalidatePath` при PUBLISHED | Точечная ревалидация при публикации |
| `/seller/[username]` | 600 сек + `revalidatePath` при изменении профиля | Редко меняется |
| `/sitemap.xml` | 3600 сек | Достаточно для поисковиков |

**Yandex.Metrika — план событий:**

| Событие | Цель | Код вызова |
|---------|------|------------|
| `prompt_view` | Просмотр страницы промпта | `ym(COUNTER_ID, 'reachGoal', 'prompt_view')` |
| `prompt_purchase_click` | Клик «Купить» | `ym(COUNTER_ID, 'reachGoal', 'purchase_click')` |
| `prompt_purchase_success` | Успешная оплата | `ym(COUNTER_ID, 'reachGoal', 'purchase_success')` |
| `free_prompt_acquire` | Получение бесплатного промпта | `ym(COUNTER_ID, 'reachGoal', 'acquire')` |
| `seller_kyc_start` | Старт KYC | `ym(COUNTER_ID, 'reachGoal', 'kyc_start')` |
| `registration_complete` | Завершение регистрации | `ym(COUNTER_ID, 'reachGoal', 'register')` |

Yandex.Metrika подключается через `next/script strategy="afterInteractive"` только при `localStorage['cookie_consent'] === 'accepted'`. При отсутствии согласия — скрипт не загружается.

---

## 18. Инфраструктура и деплой

**CI Backend:** push/PR → `pytest` + `Bandit` + `Semgrep` + `pip-audit` + `ruff` + `mypy` + `docker build`.

**CI Frontend:** выполняется параллельно с backend CI:
```yaml
# .github/workflows/ci-frontend.yml (фрагмент)
- run: npm ci
- run: npx eslint . --max-warnings 0           # 0 предупреждений — строго
- run: npx tsc --noEmit                        # type-check без компиляции
- run: npx vitest run                          # unit-тесты
- run: npx next build                          # production build
- name: Bundle budget check
  run: |
    SIZE=$(du -sk .next/static/chunks/*.js | awk '{sum+=$1} END{print sum}')
    [ "$SIZE" -le 307200 ] || (echo "Bundle > 300KB gzip limit" && exit 1)
- run: npx @axe-core/cli http://localhost:3000  # a11y smoke test (после next start)
```

**Bundle budget:** суммарный размер JS-чанков ≤ 300 KB (gzip). Превышение — CI fail. Мониторинг через `next build` output `Route (app)` таблицу и `bundleanalyzer` (по необходимости).
...
### 19.2. Frontend переменные окружения (NEXT_PUBLIC_*)

Переменные с префиксом `NEXT_PUBLIC_` инлайнятся в клиентский бандл Next.js. **Не передавать сюда секреты.**

```bash
# .env.local (dev) / GitHub Secrets → .env.production (CI/CD build step)

NEXT_PUBLIC_API_URL=https://api.promptspace.ru      # базовый URL Django API
NEXT_PUBLIC_CAPTCHA_SITE_KEY=                        # Turnstile / SmartCaptcha site key (публичный)
NEXT_PUBLIC_YANDEX_CLIENT_ID=                        # OAuth Yandex (публичный client_id)
NEXT_PUBLIC_VK_CLIENT_ID=                            # OAuth VK
NEXT_PUBLIC_TELEGRAM_BOT_USERNAME=PromptSpaceBot     # для кнопки «Подключить Telegram»
NEXT_PUBLIC_YM_COUNTER_ID=                           # Yandex.Metrika счётчик (числовой ID)
NEXT_PUBLIC_SENTRY_DSN=                              # Sentry DSN для frontend; публичен по дизайну
```

> [!NOTE]
> `NEXT_PUBLIC_SENTRY_DSN` по дизайну Sentry — публичный. Доступ к проекту защищён API-ключами на стороне Sentry. Передавать в значение без опасений.
...
### 23.7. Browser (In-App) уведомления — Frontend реализация

**Стратегия MVP: Long Polling.**

WebSocket / SSE — вне MVP: требуют дополнительной инфраструктуры (Django Channels + Redis pub/sub). Polling на 30 сек — достаточно для MVP при нагрузке до 5000 пользователей.

**Реализация (Zustand + TanStack Query):**
...
export function useNotificationPolling() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const res = await fetch('/api/v1/users/me/notifications/?is_read=false&limit=5')
      const data = await res.json()
      setUnreadCount(data.count)
      return data
    },
    refetchInterval: 30_000,          // polling каждые 30 сек
    refetchIntervalInBackground: false, // не поллить, если вкладка не активна
    enabled: !!useAuthStore.getState().accessToken,  // только для авторизованных
  })
}
```

**UI — Bell Icon с badge:**
```tsx
...
export function NotificationBell() {
  ...
  return (
    <button className="relative" aria-label={`Уведомления (${unreadCount} непрочитанных)`}>
...
**Переход на SSE (v2):** при росте пользователей заменить polling на `GET /users/me/notifications/stream/` (Server-Sent Events). Клиентский код: заменить `useQuery` + `refetchInterval` на `EventSource`. Backend: Django Channels + `asyncio` consumer. Изменения изолированы в `hooks/useNotifications.ts`.
...
> [!CAUTION]
> **Auto-Unseal зависит от Yandex KMS.** Убедиться в доступности Yandex KMS в RTO-window до начала restore.
...
### 25.9. Атомарный переход в PUBLISHED: `published_at` + `content_hash` (M-5)

При переходе промпта в статус `PUBLISHED` необходимо в **одной атомарной операции** зафиксировать `published_at` и вычислить `content_hash`. Нарушение порядка создаёт ситуацию, когда хеш содержит один timestamp, а поле `published_at` хранит другой (при retry Celery-задачи в другую секунду).

```python
from django.utils.timezone import now as utcnow
import hashlib

def publish_prompt(prompt_id: UUID, decrypted_content: str) -> None:
    """Атомарный переход в PUBLISHED с фиксацией Digital Provenance."""
    prompt = Prompt.objects.select_for_update().get(pk=prompt_id)
    if prompt.status == PromptStatus.PUBLISHED:
        return  # идемпотентность

    # Шаг 1: зафиксировать timestamp ДО вычисления хеша
    ts = utcnow()

    # Шаг 2: вычислить хеш с тем же timestamp, что будет сохранён в поле
    raw = f"{decrypted_content}{prompt.seller.seller_profile.inn}{ts.isoformat()}"
    content_hash = hashlib.sha256(raw.encode()).hexdigest()

    # Шаг 3: единый .update() — оба поля записываются в одном SQL UPDATE
    Prompt.objects.filter(pk=prompt_id).update(
...
**Правило:** никогда не вызывать `prompt.save()` после отдельного `prompt.published_at = ...` — используй только `Prompt.objects.filter(...).update(...)` для атомарности. `select_for_update()` предотвращает конкурентное изменение записи.

### 25.10. N+1 запросы в каталоге (M-4)
...
### 25.11. Политика ротации секретов (M-7)
...
## 26. Дорожная карта разработки

> [!NOTE]
> Дорожная карта — ориентир, не контракт. Сроки спринтов рассчитаны на команду 2 backend + 1 frontend + 0.5 devops. Пересматривать при изменении состава команды.

Каждый спринт = 2 недели. MVP — 6 спринтов (~12 недель).

**Definition of Done (DoD) — применяется к каждому пункту:**
- [ ] Код написан и прошёл code review (минимум 1 approval)
- [ ] Unit-тесты написаны, покрытие критических путей 100%
- [ ] Функционал задеплоен на staging и протестирован вручную
- [ ] Документ обновлён (если решение отклоняется от спецификации)

---

### 26.1. Sprint 0 — Инфраструктура (Недели 1–2)

**Цель:** Development и staging среды готовы, команда разблокирована.

| Задача | Ответственный | Критерий готовности |
|--------|--------------|---------------------|
| Репозиторий: структура (`backend/`, `frontend/`, `docs/`) | Tech Lead | `git clone` → `docker compose up` работает |
| `docker-compose.yml` с PostgreSQL 16 + pgvector, Redis 7, Vault dev, PgBouncer | DevOps | `pytest` проходит локально |
| Базовый CI: ruff, mypy, pytest, Bandit | DevOps | PR Check green |
| GitHub Environments: `staging`, `production` (manual approve) | DevOps | Secrets добавлены |
| HashiCorp Vault: кластер HA на staging (3 узла), AppRole настроен | DevOps | `vault status` → initialized, unsealed |
| Базовая Django-конфигурация: `settings/base.py`, `local.py`, `staging.py`, `production.py` | Backend | `manage.py check --deploy` без критических ошибок |
| Начальные миграции: User, SellerProfile (только базовые поля) | Backend | `migrate` без ошибок |

---

### 26.2. Sprint 1 — Аутентификация + Базовый каталог (Недели 3–4)

**Цель:** пользователь может зарегистрироваться и просмотреть каталог.

| Задача | Приоритет |
|--------|-----------|
| Email + OTP вход (Redis Lua, brute-force защита) | P0 |
| OAuth: Yandex ID | P0 |
| JWT access + refresh (rotation, blacklist) | P0 |
| Модели: Prompt, Category, Tag, PromptAccess | P0 |
| `GET /catalog/prompts` — cursor pagination, фильтрация | P0 |
| `GET /catalog/prompts/{slug}` — публичная карточка | P0 |
| Next.js: главная страница, каталог, карточка промпта (SSR) | P0 |
| OAuth: VK ID | P1 |
| OAuth: Telegram Auth Widget | P1 |

---

### 26.3. Sprint 2 — Промпты + Платежи (Недели 5–6)

**Цель:** продавец может создать промпт; покупатель — купить его.

| Задача | Приоритет |
|--------|-----------|
| Envelope Encryption: DEK генерация, Vault Transit API | P0 |
| Seller: CRUD промптов (DRAFT → submit → PENDING_MODERATION) | P0 |
| SECURITY_SCAN заглушка (pass-through для MVP) → подключить AISP в Sprint 5 | P0 |
| Robokassa Split: создание заказа, webhook | P0 |
| PromptAccess при SUCCESS | P0 |
| Бесплатный промпт: acquire + captcha | P0 |
| Расшифровка контента (порядок 25.8) | P0 |
| YOS: presigned upload URL + confirm endpoint | P1 |
| Seller KYC: Dadata API (IP / Самозанятый) | P1 |

---

### 26.4. Sprint 3 — Поиск + Уведомления (Недели 7–8)

**Цель:** семантический поиск работает; пользователи получают уведомления.

| Задача | Приоритет |
|--------|-----------|
| pgvector: HNSW индекс, YandexGPT Embeddings v3 | P0 |
| `GET /catalog/search` — offset pagination, max 1000 | P0 |
| ISearchProvider + PgVectorSearchProvider | P0 |
| Plagiarism check (cosine similarity >95% → PENDING_PLAGIARISM_REVIEW) | P0 |
| Система уведомлений: Email (Unisender), in-app | P0 |
| Telegram Bot: optional channel, onboarding flow | P1 |
| Celery Beat: `django-celery-beat`, базовое расписание | P0 |
| `aggregate_seller_stats`, `verify_active_kyc_status` | P1 |

---

### 26.5. Sprint 4 — Модерация + Безопасность (Недели 9–10)

**Цель:** модераторы могут работать; права и защиты на месте.

| Задача | Приоритет |
|--------|-----------|
| Модерация: очередь, approve, reject (с причиной) | P0 |
| PENDING_PLAGIARISM_REVIEW: resolve flow | P0 |
| BLOCKED_BY_SECURITY + APPEAL_REQUESTED flow | P0 |
| `anonymize_account` (152-ФЗ, право на забвение) | P0 |
| Data Export (`GET /users/me/export`) | P0 |
| Refund: Django Admin action + Robokassa API | P0 |
| AuditLog: партиционирование, `pg_partman` | P1 |
| Rate limiting: ADR-009 (redis-py, per-endpoint) | P0 |
| Digital Provenance: `content_hash` | P1 |
| API Keys для агентов (Agentic AI, раздел 5.7) | P2 |

---

### 26.6. Sprint 5 — Производительность + Запуск (Недели 11–12)

**Цель:** готово к production. Все load tests проходят. Чеклист выполнен.

| Задача | Приоритет |
|--------|-----------|
| SBOM (CycloneDX) + Cosign подписание образов | P0 |
| OpenTelemetry: trace_id, span_id в Django и Celery | P0 |
| Zabbix Agent + алерты таблицы 20.2 | P0 |
| Sentry SDK (Django + Next.js) | P0 |
| AISP Security Scan (реальный провайдер) | P0 |
| Load testing (Locust) на staging — пороги раздела 15.5 | P0 |
| OWASP ZAP на staging | P0 |
| CDN для публичных превью (Yandex CDN) | P1 |
| Confidential VMs для Django/Celery workers | P1 |
| Выполнение чеклиста раздела 27 | P0 |

---

## 27. Чеклист запуска в Production

> [!IMPORTANT]
> Все пункты должны быть выполнены **до** первого production deploy. Ответственный за выполнение фиксирует своё имя и дату напротив каждого пункта.

### 27.1. Инфраструктура

- [ ] HashiCorp Vault HA кластер (3 узла): инициализирован, Auto-Unseal через Yandex KMS настроен
- [ ] Ежедневный snapshot Vault в YOS: проверён restore (`vault operator raft snapshot restore`)
- [ ] PostgreSQL: streaming replica настроена, WAL archiving через pgBackRest включён
- [ ] Redis Sentinel / Cluster: минимум 3 узла, тест failover выполнен
- [ ] PgBouncer: `pool_mode=transaction`, `DATABASE_DIRECT_URL` в env для migrate
- [ ] CDN перед YOS: публичные превью раздаются через CDN
- [ ] WAF / Anti-DDoS (Yandex Cloud Shield): активирован, правила настроены
- [ ] Confidential VMs для Django + Celery workers (AMD SEV или Intel TDX)
- [ ] Nginx: TLS 1.3, HSTS, Brotli/Gzip compression включены
- [ ] Docker-образы: подписаны Cosign, policy `verify-image-signatures` включена
- [ ] SBOM (CycloneDX) сгенерирован, сохранён в artifact registry

### 27.2. Безопасность

- [ ] `DEBUG=False` в production env
- [ ] `DJANGO_NINJA_DOCS_URL=` (пустое) — OpenAPI docs отключены
- [ ] `SECRET_KEY` уникален, ≥ 50 символов, сгенерирован `python -c "import secrets; print(secrets.token_urlsafe(50))"`
- [ ] Все OAuth client secrets добавлены в GitHub Secrets, не в `.env` файлах репозитория
- [ ] `VAULT_TOKEN` не используется в production — только AppRole (`VAULT_ROLE_ID` + `VAULT_SECRET_ID`)
- [ ] Robokassa: `ROBOKASSA_IP_WHITELIST` заполнен реальными IP Robokassa
- [ ] Bandit + Semgrep: нет нерешённых `HIGH`/`CRITICAL` находок
- [ ] pip-audit + npm audit: нет нерешённых `HIGH`/`CRITICAL` CVE
- [ ] OWASP ZAP (DAST) на staging: нет нерешённых `HIGH`/`CRITICAL` находок
- [ ] Nginx: `server_tokens off`, `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Content-Security-Policy` настроен
- [ ] API Keys: хеши в БД (SHA-256), plaintext не логируется

### 27.3. Данные и 152-ФЗ

- [ ] Уведомление в Роскомнадзор об обработке ПДн подано
- [ ] DPA подписаны: Yandex Cloud, Selectel, Dadata, Unisender, Robokassa
- [ ] Страница `/legal/dsr/` опубликована, форма работает
- [ ] Политика обработки ПДн опубликована, ссылка в футере
- [ ] Cookie-баннер: Яндекс.Метрика не инициализируется до клика «Согласен»
- [ ] `anonymize_account` протестирована на staging: все 7 шагов выполнены корректно
- [ ] `GET /users/me/export` — JSON-архив содержит все данные пользователя
- [ ] Telegram-сообщения проверены: ни одно не содержит ПДн

### 27.4. Мониторинг

- [ ] Zabbix Agent установлен на всех узлах; Zabbix Server получает метрики
- [ ] Все алерты из таблицы 20.2 настроены и протестированы (тест-алерт отправлен)
- [ ] Sentry DSN настроен в Django и Next.js; тестовое исключение зафиксировано
- [ ] OpenTelemetry: trace_id виден в Sentry и структурных логах
- [ ] `GET /api/v1/health/` возвращает `{"status":"ok"}` для db, redis, vault
- [ ] Telegram-бот для алертов: тестовое сообщение получено командой

### 27.5. Тестирование

- [ ] `pytest --cov`: итоговое покрытие ≥ 80%; все критические пути — 100%
- [ ] E2E (Playwright): все 8 обязательных сценариев (раздел 15.3) проходят на staging
- [ ] Load testing (Locust): все пороги раздела 15.5 выполнены на staging с production-like данными
- [ ] Celery Beat: все периодические задачи запустились хотя бы один раз в staging

### 27.6. Операционная готовность

- [ ] `docs/runbook.md` написан: разделы Vault / Redis / PostgreSQL / Robokassa webhook
- [ ] Rollback plan задокументирован: предыдущий image tag зафиксирован
- [ ] On-call ответственный назначен; контакты добавлены в runbook
- [ ] Бэкап восстановление проверено: `pgbackrest restore` на тестовом инстансе прошло
- [ ] Первый deploy: manual approve в GitHub Environments → `production` получен
