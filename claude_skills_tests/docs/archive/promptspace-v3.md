...
> **Версия:** 0.3 (доработано по рекомендациям системного архитектора)

**Изменения v0.3 (относительно v0.2):** Circuit Breaker для автоматического KMS failover, проксирование приватных медиафайлов с валидацией JWT, бизнес-логика транзакций отвязана от ID платёжного шлюза (Vendor Lock-in), резервный канал доставки OTP перенесён в MVP.
...
**Важно:** Вся внутренняя бизнес-логика (создание Purchase, webhook-обработчики) использует универсальные UUID транзакций платформы. Специфичные поля шлюза (например, `robokassa_invoice_id`) инкапсулированы в payment service и не проникают в ядро — это снижает vendor lock-in и упрощает добавление альтернативных провайдеров.
...
class Purchase(models.Model):
    ...
    robokassa_invoice_id = models.CharField(max_length=128, unique=True)  # инкапсулирован в payment service
...
│   │   ├── kms.py
│   │   ├── kyc.py
│   │   ├── payment.py      # абстракция платежей, инкапсуляция robokassa_invoice_id
│   │   ├── search.py
│   │   ├── storage.py      # proxy для приватных медиа + presigned для публичных
│   │   └── ratelimit.py
│   ├── tasks/
│   │   ├── embeddings.py
│   │   ├── kyc.py
...
> ADR упорядочены по номеру (001 → 012). Детали Nginx и rate limiting — в ADR-009.
...
1. Buyer нажимает "Купить" → Django создаёт `Purchase(PENDING)` + Robokassa ссылку.
2. Редирект на Robokassa → Оплата.
3. Robokassa POST на ResultURL (webhook).
4. Django проверяет IP whitelist + криптографическую подпись.
5. Идемпотентное обновление `Purchase(SUCCESS)` + создание `PromptAccess` через платформенные UUID.
6. Деньги автоматически разделяются Robokassa: платформе + продавцу напрямую.

### ADR-004: Комплаенс 152-ФЗ
- Алгоритм анонимизации: см. [раздел 2.5](#25-алгоритм-анонимизации-право-на-забвение).

### ADR-005: Frontend архитектура
**Next.js (App Router)** + Django Ninja API. SEO и CWV из коробки за счёт SSR.

### ADR-006: Async Processing Pipeline (Celery + Redis)
Celery + Redis. Задачи (генерация эмбеддингов, OTP, вебхуки Robokassa) идемпотентны, асинхронны с retry.

### ADR-007: Хранение медиафайлов (Yandex Object Storage)
- **Публичные превью:** через CDN с кешированием.
- **Приватный контент:** вместо presigned URL (которые могут утечь) — **запросы проксируются через Backend**, который валидирует JWT и контролирует доступ в реальном времени. Presigned URL — только для публичных превью.

### ADR-008: Email-провайдер и уведомления
- «Сброс пароля» заменён на «Восстановление доступа» — единый OTP-флоу.
- **Fallback OTP в MVP:** при недоступности Unisender — автоматический резервный канал: SMTP.ru или Telegram Bot (если привязан). Celery retry (3 попытки, exponential backoff) — только для некритичных фоновых уведомлений.

### ADR-009: Rate Limiting (защита API)
- Двухуровневая защита: Nginx (IP) + Django/Redis (per-user).
- **Redis недоступен:** fail open; логирование WARNING; алерт в Telegram при недоступности > 1 мин.
...
**Django per-user лимиты:** OTP request 3/10мин, OTP verify 5/5мин, payment 10/час, prompt decrypt 100/час.

### ADR-010: Backup стратегия PostgreSQL
- **RPO ≤ 1 мин** (при `archive_timeout=60`), **RTO < 30 мин**.
- pgBackRest + WAL archiving в YOS.

### ADR-011: Staging окружение и Feature Flags
Staging = те же Docker-образы что production; feature flags через django-flags.

### ADR-012: Failover KMS (Circuit Breaker)
- **Основной:** Yandex KMS. **Fallback:** HashiCorp Vault (AES-256-GCM).
- **Circuit Breaker:** при превышении порога ошибок от Yandex KMS — автоматическое переключение на Vault **без рестарта** и без сброса DEK-кеша в Redis, что предотвращает thundering herd на fallback.
...
### 12.3. E2E
- Playwright для критичных сценариев: регистрация, покупка, доступ к промпту
- Запуск в CI на merge в main (опционально)

### 12.4. Credentials
- Секреты — через env или `.env.test`
- Robokassa: sandbox. KMS: mock или Vault dev mode
...
**Горизонтальное масштабирование:** реплики Django за Nginx, Celery workers, Redis single node (или cluster).

---

## 14. SEO и базовые страницы

UI Mobile-First, Core Web Vitals (LCP < 2.5s).
...
- При breaking changes: ввод **v2**, v1 deprecated ≥ 6 мес, затем отключение.
...
| KMS недоступен | ошибка при запросе DEK | Telegram, проверка KMS / Vault |
| Redis недоступен | connect timeout | Telegram, fail open для rate limit |
| Unisender API error | 5xx или таймаут | Telegram, проверка fallback (SMTP/Telegram) |
...
**Runbook:** отдельный документ. Эскалация: Telegram → DevOps.
...
| Ревью | Одна запись на (buyer, prompt); требуется PromptAccess |
| API versioning | v1 — текущий контракт; v2 при breaking changes, deprecation ≥ 6 мес |

### Актуальные риски

| ID | Риск | Severity | Митигация |
|----|------|----------|-----------|
| R1 | KMS недоступен | High | DEK-кеш Redis + Circuit Breaker failover Vault (ADR-012) |
...
| R6 | Unisender недоступен | Medium | Синхронный SMTP/Telegram fallback в MVP; retry для фоновых задач |
| R7 | Vendor Lock-in платёжного шлюза | Medium | Платформенные UUID для Purchase; абстракция payment service |
